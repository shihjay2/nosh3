    ...
    <q-header elevated>
      <q-toolbar>
        ...
        <div class="q-pr-md">
          ...
          <v-offline @detected-condition="checkOnline" v-if="!state.sync_on">
...
export default defineComponent({
  ...
  setup () {
    ...
    onMounted(async() => {
      ...
      try {
        ...
        if (route.params.id !== 'new') {
          try {
            const result = await patientDB.find({selector: {_id: {$eq: route.params.id}}, limit: 0})
    ...
    const loadTimeline = async(rebuild=false) => {
      const timelineDB = new PouchDB(prefix + 'timeline')
      const result = await timelineDB.allDocs({
        include_docs: true,
        attachments: true,
      ...
      if (!rebuild) {
        ...
        if (auth.timeline_build || result.rows.length == 0 || build) {
          ...
          for (const resource of timelineResources) {
            ...
            try {
              const result = await db.find({selector: {[base.patientField]: {$eq: 'Patient/' + state.patient }, _id: {"$gte": null}}, limit: 0})
              if (resource !== 'observations') {
                for (const a in result.docs) {
                  ...
                  if (resource === 'encounters') {
                    ...
                    const bundle_result = await bundle_db.find({selector: {'entry': {"$elemMatch": {"resource.encounter.reference": 'Encounter/' + objectPath.get(result, 'docs.' + a + '.id')}}, _id: {"$gte": null}}, limit: 0})
                    ...
                    if (objectPath.has(result, 'docs.' + a + '.sync_id')) {
                      ...
                      const doc_ref_db_res = await doc_ref_db.find({selector: {'context.encounter.0.reference': {'$regex': objectPath.get(result, 'docs.' + a + '.sync_id')}, _id: {"$gte": null}}, limit: 0})
    ...
    const openPage = async(id, resource, category) => {
      ...
      if (state.resource == 'encounters') {
        ...
        const results = await a.find({selector: {'entry.0.resource.encounter.reference': {$eq: 'Encounter/' + id}, _id: {"$gte": null}}, limit: 0})
        if (results.length > 0) {
    ...
    const setCompositionSection = async(resource) => {
      if (objectPath.has(state, 'compositionDoc.id')) {
        ...
        const result = await a.find({selector: {[b.activeField]: {$ne: 'inactive'}, _id: {"$gte": null}}, limit: 0})
        text = '<ul>'
    ...
    const signEncounter = async() => {
      ...
      for (const resource of resources) {
        ...
        const db = new PouchDB(prefix + resource)
        const results = await db.find({selector: {[base.indexField]: {$eq: [base.indexRoot] + '/' + state.encounter}, _id: {"$gte": null}}, limit: 0})
