...
export function common() {
  ...
  const eventAdd = async(event, patient_id, opts={doc_db: null, doc_id: null, diff: null}) => {
    ...
    const check1 = await db1.find({selector: {'datetime': {"$eq": datetime}}, limit: 1000})
    let doc = {
      _id: 'nosh_' + uuidv4(),
      datetime: datetime,
  ...
  const getSignedEncounters = async(yearback="100") => {
    ...
    const encountersResult = await encountersDB.find({selector: {'subject.reference': {$eq: 'Patient/' + auth_store.patient }, 'period.start': { "$gte": period }, _id: {"$gte": null}}, limit: 1000})
    if (encountersResult.docs.length > 0) {
      for (const a of encountersResult.docs) {
        const bundlesDB = new PouchDB(prefix + 'bundles')
        const bundlesResult = await bundlesDB.find({selector: {'entry.0.resource.encounter.reference': {$eq: 'Encounter/' + a.id}, _id: {"$gte": null}}, limit: 1000})
  ...
  const inbox = async(resource, user) => {
    ...
    if (resource === 'communications') {
      ...
      result = await localDB.find({selector: selector, limit: 1000})
    }
    return result
  }
  ...
  const patientList = async(user) => {
    ...
    const patientList = await patientDB.find({selector: {_id: {$regex: '^nosh_*'}}, limit: 1000})
  ...
  const sync = async(resource, online, patient_id, save=false, data={}, destroy=false) => {
    ...
    if (online) {
      ...
      const sync_result = await sync_db.find({selector: {'resource': {"$eq": resource}}, limit: 1000})
    ...
    if (destroy) {
      ...
      auth_store.setLastSync(timestamp)
      const sync_result1 = await sync_db.find({selector: {'resource': {"$eq": resource}}, limit: 1000})
  ...
  const syncEmailToUser = async(resource, category, doc, patient_id) => {
    ...
    if (resource === 'patients' ||
      ...
      const result_users = await db_users.find({selector: {'reference': {$eq: Case.pascal(pluralize.singular(resource)) + '/' + doc.id}}, limit: 1000})
      ...
      if (objectPath.has(opts, 'id')) {
        ...
        if (opts.resource === 'encounters') {
          ...
          if (objectPath.has(doc, 'sync_id')) {
            ...
            const doc_ref_db_res = await doc_ref_db.find({selector: {'context.encounter.0.reference': {'$regex': objectPath.get(doc, 'sync_id')}, _id: {"$gte": null}}, limit: 1000})
  ...
  const threadLater = async(id, arr, status, online, patient_id) => {
    ...
    if (status === 'completed') {
      ...
      const pending_result = await localDB.find({selector: pending, limit: 1000})
    ...
    const result = await localDB.find({selector: selector, limit: 1000})
    if (result.docs.length > 0) {
