...
export function common() {
  ...
  const eventAdd = async(event, patient_id, opts={doc_db: null, doc_id: null, diff: null}) => {
    ...
    const check1 = await db1.find({selector: {'datetime': {"$eq": datetime}}, limit: 0})
    let doc = {
      _id: 'nosh_' + uuidv4(),
      datetime: datetime,
  ...
  const getSignedEncounters = async(yearback="100") => {
    ...
    const encountersResult = await encountersDB.find({selector: {'subject.reference': {$eq: 'Patient/' + auth_store.patient }, 'period.start': { "$gte": period }, _id: {"$gte": null}}, limit: 0})
    if (encountersResult.docs.length > 0) {
      for (const a of encountersResult.docs) {
        const bundlesDB = new PouchDB(prefix + 'bundles')
        const bundlesResult = await bundlesDB.find({selector: {'entry.0.resource.encounter.reference': {$eq: 'Encounter/' + a.id}, _id: {"$gte": null}}, limit: 0})
  ...
  const inbox = async(resource, user) => {
    ...
    if (resource === 'communications') {
      ...
      result = await localDB.find({selector: selector, limit: 0})
    }
    return result
  }
  ...
  const patientList = async(user) => {
    ...
    const patientList = await patientDB.find({selector: {_id: {$regex: '^nosh_*'}}, limit: 0})
  ...
  const referenceSearch = async(resource, id) => {
    ...
    const results = await db.find({
      selector: {'sync_id': {$eq: id}, _id: {"$gte": null}}, limit: 0
    })
  ...
  const sync = async(resource, online, patient_id, save=false, data={}, destroy=false) => {
    ...
    if (online) {
      ...
      const sync_result = await sync_db.find({selector: {'resource': {"$eq": resource}}, limit: 0})
    ...
    if (destroy) {
      ...
      auth_store.setLastSync(timestamp)
      const sync_result1 = await sync_db.find({selector: {'resource': {"$eq": resource}}, limit: 0})
  ...
  const syncEmailToUser = async(resource, category, doc, patient_id) => {
    ...
    if (resource === 'patients' ||
      ...
      const result_users = await db_users.find({selector: {'reference': {$eq: Case.pascal(pluralize.singular(resource)) + '/' + doc.id}}, limit: 0})
  ...
  const timelineUpdate = async(opts) => {
    ...
    if (opts.action == 'delete') {
      ...
      if (objectPath.has(opts, 'id')) {
        ...
        if (opts.resource === 'encounters') {
          ...
          const bundle_result = await bundle_db.find({selector: {'entry': {"$elemMatch": {"resource.encounter.reference": 'Encounter/' + opts.id}}, _id: {"$gte": null}}, limit: 0})
    ...
    if (result.rows.length > 0) {
      ...
      const activitiesResult = await activitiesDb.find({selector: {event: {$eq: 'Chart Created' }, _id: {"$gte": null}}, limit: 0})
      const timelineIntro = {
        id: 'intro',
  ...
  const threadLater = async(id, arr, status, online, patient_id) => {
    ...
    if (status === 'completed') {
      ...
      const pending_result = await localDB.find({selector: pending, limit: 0})
    ...
    const result = await localDB.find({selector: selector, limit: 0})
    if (result.docs.length > 0) {
