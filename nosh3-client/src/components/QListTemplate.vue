...
export default defineComponent({
  ...
  setup (props, { emit }) {
      ...
      for (const a in rows) {
        ...
        if (objectPath.has(rows, a + '.doc.telecom')) {
          ...
          if (email !== undefined) {
            ...
            const result_users = await localDB5.find({selector: {'email': {$eq: email.value}, _id: {"$gte": null}}, limit: 0})
    ...
    const getBundle = async(rows) => {
      ...
      for (const b of rows) {
        const result = await localDB3.find({selector: {'entry.0.resource.encounter.reference': {$eq: 'Encounter/' + b.id}, _id: {"$gte": null}}, limit: 0})
        if (result.docs.length > 0) {
          result.docs.sort((a1, b1) => moment(b1.timestamp) - moment(a1.timestamp))
        }
        results.push(result)
        if (objectPath.has(b, 'doc.sync_id')) {
          const result_document_reference = await localDB7.find({selector: {'context.encounter.0.reference': {'$regex': b.doc.sync_id}, _id: {"$gte": null}}, limit: 0})
    ...
    const getCarePlans = async(rows) => {
      for (const index in rows) {
        const result = await localDB1.find({selector: {'contained.0.id': {$eq: rows[index].id}, _id: {"$gte": null}}, limit: 0})
    ...
    const getMedicationCarePlan = async(rows) => {
      for (const index in rows) {
        const result = await localDB1.find({selector: {'activity': {"$elemMatch": {"reference": "MedicationStatement/" + rows[index].id}}, _id: {"$gte": null}}, limit: 0})
        if (objectPath.has(result, 'docs.0')) {
          for (const d in objectPath.get(result, 'docs')) {
    ...
    const getMedicationRequests = async(rows) => {
      for (const index in rows) {
        if (objectPath.has(rows, index + '.doc.medicationCodeableConcept.coding.0.code')) {
          const result = await localDB4.find({selector: {'medicationCodeableConcept.coding.0.code': {$eq: rows[index].doc.medicationCodeableConcept.coding[0].code}, _id: {"$gte": null}}, limit: 0})
          if (objectPath.has(result, 'docs.0')) {
