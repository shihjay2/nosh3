      ...
      if (state.careplan == true) {
          ...
          if (id !== '') {
            if (doc.code.coding[0].code === '29463-7' || doc.code.coding[0].code === '8302-2') {
              ...
              if (doc.code.coding[0].code === '29463-7') {
                ...
                const check = await checkDB.find({selector: {'encounter.reference': {$eq: 'Encounter/' + props.encounter}, 'code.coding.0.code': {$eq: '8302-2'}, _id: {"$gte": null}}, limit: 1000})
                ...
                const check = await checkDB.find({selector: {'encounter.reference': {$eq:  'Encounter/' + props.encounter}, 'code.coding.0.code': {$eq: '29463-7'}, _id: {"$gte": null}}, limit: 1000})
                if (check.docs.length > 0) {
              ...
              if (weight !== 0 && height !== 0) {
                ...
                const check1 = await checkDB.find({selector: {'encounter.reference': {$eq: 'Encounter/' + props.encounter}, 'code.coding.0.code': {$eq: '39156-5'}, _id: {"$gte": null}}, limit: 1000})
        ...
        if (state.resource == 'service_requests') {
          ...
          if (id !== '') {
            ...
            const task = await taskDB.find({selector: {'basedOn.0.reference': {$eq: 'ServiceRequest/' + id}, _id: {"$gte": null}}, limit: 1000})
