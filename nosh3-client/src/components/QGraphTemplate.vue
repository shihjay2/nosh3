...
export default defineComponent({
  ...
  setup(props, { emit }) {
    ...
    const dft = async(observation) => {
      const series = []
      const result = await db.find({selector: {'code.coding.0.code': {$eq: observation}, _id: {"$gte": null}}, limit: 10000})
      const data1 = []
    ...
    const gc = async(type) => {
      ...
      const result = await db.find({selector: {$or: selector}, limit: 10000})
      const data1 = []
      const data2 = []
      result.docs.sort((a1, b1) => moment(a1.effectivePeriod.start) - moment(b1.effectivePeriod.start))
      for (const d in result.docs) {
